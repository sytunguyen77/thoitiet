...
the city name with template literals*/
    fetch(
        `https://api.weatherapi.com/v1/current.json?key=74b3217f465b4854b0280255222012&q=${cityInput}`
    )
  ...
  and convert it to a regular JS object*/
    ...
    and weather condition to the page*/
            temp.innerHTML = Math.round(data.current.temp_c) + "&#176" + "C";
            tempC.style.color = "#fff";
            tempF.style.color = "#aaa";
            conditionOutput.innerHTML = data.current.condition.text;

            /*Switching from Celcius to Fahrenheit*/
            function changeTemp() {
                temp.innerHTML =
                    Math.round(data.current.temp_f) + "&#176" + "F";
            }

            tempF.addEventListener("click", () => {
                tempF.style.color = "#fff";
                tempC.style.color = "#aaa";
                changeTemp();
            });
    ...
    local folder path and add it to the page*/
      ...
      depending on if its day or night*/
                ...
                video.src = `./video/${timeOfDay}/rainy.mp4`;
